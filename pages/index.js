...
export default function LandingPage() {
	...
	useEffect(() => {
		...
		if (user) {
			...
			if (type == "moderator")
				return router.push(`/institutions?user=${user.id}`);
	...
	const responseGoogle = async (response) => {
		const request = await fetch(
			process.env.BACKEND_API_UR + "/auth/convert-token",
			{
				method: "POST",
				headers: {
				...
				<section className={styles.hero}>
					<div>
						<h1 className={styles.hero__heading}>
							Level Up Your Research Experience
						</h1>
						<p className={styles.hero__text}>
							Meegu is a online platform that helps you make your research
							journey easier and more meaningful by connecting you to all actors
							of research so you can have a much easier and more meaningful in a
							system that works!
						</p>
					</div>
